  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        if (profileData) {
          // Profile already exists, store agentId and redirect to reporchestrator
          if (profileData._id) {
            Cookies.set('agentId', profileData._id);
            localStorage.setItem('agentId', profileData._id);
            
            // Redirect to reporchestrator if not already redirecting
            if (!hasNavigated.current) {
              hasNavigated.current = true;
              router.push('/reporchestrator');
              return;
            }
          }
          ...
          setGeneratedSummary(profileData.professionalSummary?.profileDescription || '');
            ...
            // Profile not found (404) or network error - treat as new profile
            // Open import dialog to allow user to create a profile
            console.log("No profile found, opening import dialog for new profile creation");
