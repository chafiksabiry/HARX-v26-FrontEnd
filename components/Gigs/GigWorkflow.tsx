...
import { GigStatus } from '../../lib/gigs/gigStatus';

interface GigWorkflowProps {
  gig: GigData;
  onUpdate: (gig: GigData) => void;
}
...
export function GigWorkflow({ gig, onUpdate }: GigWorkflowProps) {
  ...
  const handleSubmitForReview = async () => {
    try {
      ...
      const updatedGig = { ...gig, status: 'to_activate' as GigStatus };
      onUpdate(updatedGig);
    } catch (err) {
  ...
  const handlePublish = async () => {
    try {
      setIsLoading(true);
      setError(null);
      // TODO: Implement API call to publish gig
      const updatedGig = { ...gig, status: 'active' as GigStatus };
      if (updatedGig) {
        onUpdate(updatedGig);
      }
    } catch (err) {
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="capitalize">{gig.status?.replace('_', ' ') || 'Unknown'}</span>
        </div>
