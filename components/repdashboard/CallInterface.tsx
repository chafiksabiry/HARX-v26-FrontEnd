...
export function CallInterface({ phoneNumber, agentId, onEnd, onCallSaved, provider = 'twilio', keepAIPanelAfterCall = true, callId }: CallInterfaceProps) {
    ...
    const initiateCall = async () => {
      ...
      try {
        if (provider === 'qalqul') {
          ...
          const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL_CALL || ''}/api/calls/token`);
            ...
            setTimeout(() => {
              ...
              if (stream) {
                try {
                  ...
                  const apiUrlCall = process.env.NEXT_PUBLIC_API_URL_CALL || '';
                  const wsUrl = process.env.NEXT_PUBLIC_WS_URL || `${apiUrlCall.replace('http', 'ws')}/speech-to-text`;
  ...
  const handleTranscription = async (transcription: string) => {
    ...
    try {
      ...
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL_CALL || ''}/api/calls/ai-assist`;
