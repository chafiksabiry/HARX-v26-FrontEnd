...
export function CallRecords({ agentId: propAgentId }: CallRecordsProps = {}) {
  ...
  useEffect(() => {
    ...
    const fetchCallRecords = async () => {
      try {
        // Try to get agentId from props, cookies, or localStorage
        const agentId = propAgentId || Cookies.get('agentId') || (typeof window !== 'undefined' ? localStorage.getItem('agentId') : null);
        if (!agentId) {
        ...
        // Try to fetch calls by agent ID first, fallback to all calls
        let agentCalls = [];
        try {
          ...
          if (response.data?.success && response.data.data) {
            agentCalls = response.data.data;
          } else if (Array.isArray(response.data)) {
            agentCalls = response.data;
          }
        } catch (agentError: any) {
          // If agent-specific endpoint doesn't exist, fetch all calls and filter
          console.log('Agent-specific endpoint not available, fetching all calls:', agentError);
          const response = await api.get('/calls');
          console.log("📞 All calls response:", response);
          ...
          // Filter calls by agentId
          agentCalls = calls.filter((call: any) => {
            const callAgentId = call.agent || call.agentId || call.agent?._id;
            return callAgentId === agentId || callAgentId?.toString() === agentId.toString();
          });
        }
        
        console.log(`✅ Successfully retrieved ${agentCalls.length} call records for agent`);
        setCallRecords(agentCalls);
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <button 
                        ...
                        onClick={() => {
                          ...
                          // Store call data in sessionStorage for the next page
                          if (typeof window !== 'undefined') {
                            sessionStorage.setItem('selectedCall', JSON.stringify(record));
                          }
                          router.push(`/repdashboard/call-report?id=${record._id}`);
                        }}
                      >
