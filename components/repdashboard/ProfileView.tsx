...
export const ProfileView: React.FC<{ profile: any, onEditClick: () => void }> = ({ profile, onEditClick }) => {
      ...
      try {
        ...
        if (profile?.personalInfo?.country) {
          if (typeof profile.personalInfo.country === 'string') {
            ...
            // Country is already an object, normalize it to frontend format
            const countryObj = profile.personalInfo.country;
            // Handle backend Country format: { name: { common, official }, cca2 }
            const normalizedCountry = {
              _id: countryObj._id || countryObj.id || '',
              countryName: countryObj.countryName || countryObj.name?.common || countryObj.name?.official || countryObj.name || '',
              countryCode: countryObj.countryCode || countryObj.iso2 || '',
              name: countryObj.countryName || countryObj.name?.common || countryObj.name?.official || (typeof countryObj.name === 'string' ? countryObj.name : ''),
              code: countryObj.countryCode || countryObj.iso2 || '',
              ...countryObj
            };
            setCountryData(normalizedCountry);
          }
        }
  ...
  useEffect(() => {
    const checkMismatch = async () => {
      ...
      try {
        ...
        console.log('🔍 Checking country mismatch for selected country:', countryCode);
  ...
  const getTimezoneMismatchInfo = () => {
    ...
    if (timezoneCountry !== selectedCountryCode) {
      ...
      const selectedCountryName = typeof countryData.countryName === 'string' 
        ? countryData.countryName 
        : (typeof countryData.name === 'object' && countryData.name !== null && 'common' in countryData.name
          ? ((countryData.name as any).common || (countryData.name as any).official || '')
          : (typeof countryData.name === 'string' ? countryData.name : ''));
  ...
  const takeLanguageAssessment = (language: string, iso639_1Code?: string) => {
    ...
    const isStandaloneMode = process.env.NEXT_PUBLIC_RUN_MODE === 'standalone';
    // Use the appropriate assessment app URL based on the mode
    const assessmentAppUrl = isStandaloneMode 
      ? process.env.NEXT_PUBLIC_ASSESSMENT_APP_STANDALONE 
      : process.env.NEXT_PUBLIC_ASSESSMENT_APP;
        ...
        <div className="bg-white rounded-lg p-6">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-2 text-gray-600 mb-4">
              ...
              <span>
                {countryData?.countryName 
                  ? (typeof countryData.countryName === 'string' 
                      ? countryData.countryName 
                      : 'Country not specified')
                  : (typeof countryData?.name === 'object' && countryData?.name !== null && 'common' in countryData.name
                    ? ((countryData.name as any).common || (countryData.name as any).official || 'Country not specified')
                    : (typeof countryData?.name === 'string' ? countryData.name : 'Country not specified'))}
              </span>
            </div>
                      ...
                      : typeof profile.availability.timeZone === 'string'
                        ...
                        : typeof profile.availability.timeZone === 'object' && profile.availability.timeZone
                          ? (() => {
                              ...
                              const tzCountryName = typeof tz.countryName === 'string' 
                                ? tz.countryName 
                                : (tz.countryName?.common || tz.countryName?.official || tz.name?.common || tz.name?.official || '');
                              const tzZoneName = tz.zoneName || tz.name || '';
                              const tzOffset = tz.gmtOffset || tz.offset || 0;
                              return `${tzCountryName} - ${tzZoneName} (GMT${tzOffset >= 0 ? '+' : ''}${tzOffset})`;
                            })()
                          : 'Not specified'
                    }
                  </span>
