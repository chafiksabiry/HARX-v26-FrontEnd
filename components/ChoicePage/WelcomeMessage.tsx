...
import { auth } from '@/lib/api'; // Use unified auth API client
...
export function WelcomeMessage({ type, onClose }: WelcomeMessageProps) {
  ...
  const handleContinue = async () => {
    ...
    try {
      // Update userType in the database via API
      const response = await auth.changeUserType(userId, type);
  
      if (!response || !response.success) {
        throw new Error(response?.message || 'Failed to update user type');
      }
  
      // Save new type in cookie for quick access
