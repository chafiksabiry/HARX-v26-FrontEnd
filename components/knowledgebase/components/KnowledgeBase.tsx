...
// Extended CallRecord type with runtime properties
interface ExtendedCallRecord extends CallRecord {
  processingOptions?: {
...
const KnowledgeBase: React.FC = () => {
  ...
  const getUserId = () => {
    const runMode = typeof window !== 'undefined' 
      ? (process.env.NEXT_PUBLIC_RUN_MODE || 'in-app')
      : 'in-app';
    let userId;
    ...
    if (runMode === 'standalone') {
      ...
      userId = typeof window !== 'undefined'
        ? (process.env.NEXT_PUBLIC_STANDALONE_USER_ID || undefined)
        : undefined;
  ...
  const updateOnboardingProgress = async () => {
    try {
      ...
      const apiUrl = typeof window !== 'undefined'
        ? (process.env.NEXT_PUBLIC_API_URL_ONBOARDING || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api')
        : (process.env.NEXT_PUBLIC_API_URL_ONBOARDING || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api');
  ...
  useEffect(() => {
    const fetchCallRecords = async () => {
      try {
        ...
        const calls: ExtendedCallRecord[] = response.data.callRecordings.map((call: any) => ({
          id: call.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadType === 'document') {
        ...
        const newCall: ExtendedCallRecord = {
  ...
  const cleanupAudioResources = (call: ExtendedCallRecord) => {
  ...
  const handleBackToOrchestrator = () => {
    const orchestratorUrl = typeof window !== 'undefined'
      ? (process.env.NEXT_PUBLIC_COMPANY_ORCHESTRATOR_URL || process.env.NEXT_PUBLIC_FRONT_URL || window.location.origin)
      : (process.env.NEXT_PUBLIC_COMPANY_ORCHESTRATOR_URL || process.env.NEXT_PUBLIC_FRONT_URL || '');
    if (orchestratorUrl) {
  ...
  const getUnifiedItems = () => {
    ...
    return [...documentItems, ...callItems]
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                        ...
                        <div className="flex items-center space-x-2 flex-shrink-0">
                          ...
                          {(call as ExtendedCallRecord).processingOptions?.sentiment && call.sentiment && (
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        {call.tags && call.tags.length > 0 && call.tags.map((tag: string, index: number) => (
                          <span 
                            key={`${call.id}-${tag}`}
                ...
                <div key={item.id} className="bg-white p-5 rounded-lg shadow-sm border border-gray-100">
                  <div className="flex items-start">
                    <div className="p-3 rounded-lg bg-gray-100 mr-4 flex-shrink-0">
                      {getItemIcon(item.type || undefined)}
                    </div>
                    <div className="flex-grow min-w-0">
