...
function Dashboard() {
  ...
  const repDashboardUrl = process.env.NEXT_PUBLIC_RUN_MODE === 'standalone' 
    ? process.env.NEXT_PUBLIC_REP_DASHBOARD_URL_STANDALONE || ''
    : process.env.NEXT_PUBLIC_REP_DASHBOARD_URL || '';
  const repMarketplaceUrl = process.env.NEXT_PUBLIC_REP_MARKETPLACE_URL || '';
  const router = useRouter();
  
  // For Phase 2 (Profile Creation), redirect to repcreationprofile in harx2-frontend
  const getPhase2RedirectUrl = () => {
    // If repDashboardUrl is set and it's an external URL, use it
    if (repDashboardUrl && repDashboardUrl.startsWith('http')) {
      return repDashboardUrl;
    }
    // Otherwise, use the internal route for profile creation
  ...
  const handlePhaseAction = async (phase: Phase) => {
    try {
      // For phase 2 (Profile Creation), redirect to repcreationprofile
      if (phase.id === 2) {
        const redirectUrl = getPhase2RedirectUrl();
      ...
      // For phase 3, redirect to REP dashboard if URL is provided
      if (phase.id === 3 && repDashboardUrl) {
        if (repDashboardUrl.startsWith('http')) {
          window.location.href = repDashboardUrl;
        } else {
          router.push(repDashboardUrl);
        }
        return;
      }
      ...
      if (phase.id === 4) {
        // For subscription phase, use Next.js navigation
        router.push('/reporchestrator/subscription');
      } else {
  ...
  useEffect(() => {
    ...
    // Only set userData if we have required userId and agentId
    ...
    // Set up periodic sync to check progress from the backend
  ...
  const syncProgressWithBackend = async () => {
    try {
      ...
      if (!userData.agentId) {
        console.error('No agentId available for refresh');
        return;
      }
      ...
      <div className="relative">
        ...
        <div className="space-y-8">
          {visiblePhases.map((phase, index) => {
            ...
            const isExternalLink = (phase.id === 2 && getPhase2RedirectUrl().startsWith('http')) || 
                                  (phase.id === 3 && repDashboardUrl && repDashboardUrl.startsWith('http')) || 
                                  (phase.id === 5 && repMarketplaceUrl && repMarketplaceUrl.startsWith('http'));
