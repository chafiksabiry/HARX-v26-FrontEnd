...
export const RequirementStatus: React.FC<RequirementStatusProps> = ({
  ...
  const loadRequirements = async () => {
    try {
      ...
      const group = await requirementService.checkExistingGroup(companyId, countryCode);
      setExistingGroup(group);

      if (!group) {
        // 2. Si pas de groupe, charger les requirements nécessaires
        const reqsResponse = await requirementService.checkCountryRequirements(countryCode);
        setRequirements(reqsResponse.requirements || []);
      }

      // Si le groupe existe et est actif, notifier le parent
      if (group?.status === 'active' && onRequirementsComplete) {
        onRequirementsComplete(group._id);
      }
    } catch (err) {
