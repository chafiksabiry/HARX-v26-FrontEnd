...
import { gigs as gigsApi, companies } from '@/lib/api';
...
const TelephonySetup = ({ onBackToOnboarding }: TelephonySetupProps): JSX.Element => {
    ...
    const checkGigPhoneNumber = async () => {
      ...
      try {
        ...
        if (result?.hasNumber && result.number) {
          ...
          if (provider === 'telnyx' && destinationZone) {
        ...
        console.error('âŒ Error checking gig numbers:', error);
            return false;
          }
        };
  ...
  const handleTelnyxProvider = async () => {
    ...
    if (savedGroupId) {
      ...
      try {
        ...
        // Always search for available numbers if destinationZone is available
        if (destinationZone) {
          searchAvailableNumbers();
        }
      } catch (error) {
          ...
          if (response.hasRequirements) {
            ...
            if (isNew) {
              ...
              try {
                ...
                const completedCount = detailedStatus.completedRequirements?.length || 0;
                const totalCount = detailedStatus.totalRequirements || 0;
                ...
                setRequirementStatus({
                  isChecking: false,
                  hasRequirements: true,
                  isComplete: detailedStatus.isComplete || false,
                  error: null,
                  groupId: group._id,
                  telnyxId: group.telnyxId,
                  completionPercentage,
                  ...
                  pendingRequirements: detailedStatus.pendingRequirements || 0
                });
              } catch (statusError: any) {
                console.error('❌ Error getting detailed status:', statusError);
                // Fallback: set basic status without detailed info
                setRequirementStatus({
                  isChecking: false,
                  hasRequirements: true,
                  isComplete: false,
                  error: statusError.message || 'Failed to get requirement status',
                  groupId: group._id,
                  telnyxId: group.telnyxId,
                  completionPercentage: 0,
                  completedRequirements: [],
  ...
  useEffect(() => {
    ...
    if (provider === 'telnyx') {
      ...
      // For other providers, just check for existing numbers and search available ones
      checkGigPhoneNumber();
      // Only search if destination zone is set
      if (destinationZone) {
        searchAvailableNumbers();
      }
    }
  }, [selectedGigId, destinationZone, provider]);
  ...
  const checkCompletedSteps = async () => {
    try {
      ...
      try {
        const response = await companies.getOnboarding(companyId);
        
        console.log('ðŸ"¡ API response for onboarding:', response);
        
        if (response && response.completedSteps && Array.isArray(response.completedSteps)) {
  ...
  const fetchGigs = async () => {
    ...
    try {
      ...
      const response = await gigsApi.getByCompanyId(companyId);
      console.log('✅ Gigs response:', response);
      
      // Handle both { data: Gig[] } and Gig[] response formats
      const gigsData = Array.isArray(response) ? response : (response.data || []);
      
      if (Array.isArray(gigsData) && gigsData.length > 0) {
        setGigs(gigsData);
        console.log('ðŸ“‹ Loaded gigs with populated country data:', gigsData.length);
        
        // Debug: log first gig's destination_zone
        if (gigsData.length > 0) {
          console.log('ðŸŒ First gig destination_zone:', gigsData[0].destination_zone);
        }
      } else {
        setGigs([]);
      ...
      console.error('âŒ Error fetching gigs:', error);
      setGigs([]);
    } finally {
  ...
  const fetchExistingNumbers = async () => {
    try {
      if (!selectedGigId) {
        console.error('âŒ No selectedGigId available');
        setPhoneNumbers([]);
        return;
      }
  ...
  const searchAvailableNumbers = async () => {
    ...
    console.log('ðŸ” Searching phone numbers with destination zone:', destinationZone);
  ...
  const purchaseNumber = async (phoneNumber: string) => {
    if (!selectedGigId || !companyId) {
      console.error('âŒ Required IDs missing:', { selectedGigId, companyId });
    ...
    try {
      ...
      if (provider === 'telnyx') {
        ...
        // 4. VÃ©rifier si nous avons l'ID du groupe de requirements
      ...
      console.error('âŒ Error purchasing number:', error);
      setPurchaseStatus('error');
  ...
  const handleSaveConfiguration = async () => {
    try {
      ...
      try {
        const onboardingResponse = await companies.getOnboarding(companyId);
        
        const currentCompletedSteps = onboardingResponse?.completedSteps || [];
        const newCompletedSteps = currentCompletedSteps.includes(5) ? currentCompletedSteps : [...currentCompletedSteps, 5];
        
        const updateResponse = await companies.updateOnboardingStepQuery(companyId, 2, 5, 'completed');
        ...
        try {
          const response = await companies.updateOnboardingStepQuery(companyId, 2, 5, 'completed');
  ...
  const handleSubmitRequirements = async (values: Record<string, any>) => {
    try {
      ...
      // 1. Utiliser le groupe existant ou en crÃ©er un nouveau
      ...
      console.error('âŒ Error submitting requirements:', error);
    ...
    <div className="space-y-6">
                ...
                <span className="flex items-center">
                  {selectedGigId ? (
                    (() => {
                      ...
                      return selectedGig ? (
                        <>
                           {selectedGig.title} - {selectedGig.destination_zone?.name?.common || 'Unknown'}
                          {selectedGig.destination_zone?.flags?.png && (
                            <img 
                              src={selectedGig.destination_zone.flags.png} 
                              alt={selectedGig.destination_zone.flags?.alt || 'Flag'}
                    ...
                    >
                      <div className="flex items-center">
                        <span className="text-blue-800 font-medium">
                          ðŸ“‹ {gig.title} - {gig.destination_zone?.name?.common || 'Unknown'}
                        </span>
                        {gig.destination_zone?.flags?.png && (
                          <img 
                            src={gig.destination_zone.flags.png} 
                            alt={gig.destination_zone.flags?.alt || 'Flag'}
                            className="inline-block w-6 h-4 ml-2 rounded-sm border border-gray-200 object-cover"
                          />
                        )}
                      </div>
          ...
          {destinationZone && selectedGigId && (() => {
            ...
            return selectedGig ? (
              <span className="text-sm text-gray-600">
                Destination Zone: <span className="font-medium">
        ...
        {destinationZone && provider === 'telnyx' && (
          <div className="mb-6 space-y-2">
            <h4 className="text-sm font-medium text-gray-700">Available Numbers (Destination: {(() => {
              const selectedGig = gigs.find((g: Gig) => g._id === selectedGigId);
              return selectedGig 
                ? (selectedGig.destination_zone?.name?.common || destinationZone)
                : destinationZone;
            })()})</h4>
            <div className="grid gap-2">
              {Array.isArray(availableNumbers) && availableNumbers.length > 0 ? (
      ...
      <RequirementFormModal
        ...
        existingValues={requirementStatus.completedRequirements?.map((req: RequirementDetail) => ({
          field: req.id,
          value: req.value !== undefined ? JSON.stringify(req.value) : undefined,
          status: req.status,
          submittedAt: typeof req.submittedAt === 'string'
            ? req.submittedAt
              : req.submittedAt !== undefined && req.submittedAt !== null
              ? (req.submittedAt as Date).toISOString()
