import { Rep, Gig, Match, MatchResponse, MatchingWeights, GigAgentRequest } from './matchingTypes';
...
const MATCHING_API_URL = `${BACKEND_URL}/api`;
const GIGS_API_URL = `${BACKEND_URL}/api`;

// ===== REPS API =====
export const getReps = async (): Promise<Rep[]> => {
  console.log('getReps called');
  try {
    const response = await fetch(`${MATCHING_API_URL}/matching/reps`, {
      method: 'GET',
      headers: {
...
export const getGigs = async (): Promise<Gig[]> => {
  ...
  try {
    const response = await fetch(`${GIGS_API_URL}/gigs`);
...
export const findMatchesForGig = async (gigId: string, weights: MatchingWeights): Promise<MatchResponse> => {
  ...
  try {
    const response = await fetch(`${MATCHING_API_URL}/matching/match/${gigId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    console.log('Parsed findMatchesForGig response:', responseData);
    
    // Extract data from backend response structure: { success: true, data: result }
    const data = responseData.data || responseData;
    
    // Transform backend response to frontend format
    // Backend returns: { matches: [...], totalMatches: ..., ... }
    // Frontend expects: { preferedmatches: [...], totalMatches: ..., ... }
    const matches = data.matches || [];
    
    // Calculate match statistics
    const totalMatches = matches.length;
    const perfectMatches = matches.filter((m: any) => (m.score || m.totalMatchingScore || 0) >= 0.9).length;
    const partialMatches = matches.filter((m: any) => {
      const score = m.score || m.totalMatchingScore || 0;
      return score >= 0.5 && score < 0.9;
    }).length;
    const noMatches = matches.filter((m: any) => (m.score || m.totalMatchingScore || 0) < 0.5).length;
    
    return {
      totalMatches: data.totalMatches || totalMatches,
      perfectMatches,
      partialMatches,
      noMatches,
      preferedmatches: matches, // All matches, not just qualifying ones
      matches: matches,
      ...data // Include other fields from backend (totalAgents, qualifyingAgents, etc.)
    } as MatchResponse;
  } catch (error: any) {
...
export const saveGigWeights = async (gigId: string, matchingWeights: MatchingWeights): Promise<GigWeights> => {
  ...
  try {
    const response = await fetch(`${MATCHING_API_URL}/matching/weights/${gigId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
// Returns null if no weights are found (404), throws error for other cases
export const getGigWeights = async (gigId: string): Promise<GigWeights | null> => {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        // Return null instead of throwing error - this is expected when no weights are saved
        console.log('ℹ️ No saved weights found for gig:', gigId, '- This is normal for new gigs');
        return null;
      }
      const errorData = await response.json();
    ...
    // Only log network errors, but still return null to allow default weights
    if (error instanceof Error && error.message.includes('Failed to fetch')) {
      console.warn('⚠️ Network error loading weights for gig:', gigId, '- Will use default weights');
      return null;
    }
