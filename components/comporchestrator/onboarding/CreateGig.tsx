...
const CreateGig = () => {
  ...
  // Vï¿½rifier l'ï¿½tat de l'ï¿½tape quand les donnï¿½es du formulaire changent
  useEffect(() => {
  ...
  const checkStepStatus = async () => {
    try {
      ...
      // Vï¿½rifier l'ï¿½tat de l'ï¿½tape 4 via l'API d'onboarding
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_COMPANY_API_URL}/companies/${companyId}/onboarding/step?phase=2&step=4`
      );
      ...
      if (hasBasicInfo() && !isStepCompleted) {
        ...
        // Mettre ï¿½ jour le localStorage avec l'ï¿½tape 4 marquï¿½e comme complï¿½tï¿½e
  ...
  const handlePublishGig = async () => {
    try {
      ...
      // Marquer l'ï¿½tape 4 comme complï¿½tï¿½e
