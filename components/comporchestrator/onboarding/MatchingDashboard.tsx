...
function RepMatchingPanel() {
  ...
  const handleGigSelect = async (gig: Gig) => {
    ...
    // Default weights to use if no saved weights are found
    ...
    try {
      ...
      if (savedWeights && savedWeights.matchingWeights) {
        // Use saved weights if they exist
        ...
        // No saved weights found - use default weights
      ...
      console.log("=== MATCHES DATA ===", matchesData);
        ...
        matchesData = {
          ...
          partialMatches: 0,
          noMatches: 0,
          languageStats: emptyStats,
          skillsStats: emptyStats,
          industryStats: { ...emptyStats, neutralMatches: 0 },
          activityStats: { ...emptyStats, neutralMatches: 0 },
          experienceStats: emptyStats,
          timezoneStats: emptyStats,
          regionStats: emptyStats,
          availabilityStats: emptyStats
        };
      }
      ...
      setMatches(matchesData.preferedmatches || ('matches' in matchesData ? matchesData.matches : undefined) || []);
      setMatchStats(matchesData as MatchResponse);
  ...
  const resetWeights = () => {
    // Use the same default weights as defined at the top
    setWeights(defaultWeights);
  ...
  const handleCreateGigAgent = async (match: Match) => {
    ...
    try {
      ...
      // Refresh essential data immediately to update the UI
      const companyId = Cookies.get('companyId') || '685abf28641398dc582f4c95';
      
      // Fetch only what we need - with a small delay to ensure backend has saved
      setTimeout(async () => {
        try {
          ...
          // Add rep to invited list for immediate UI update
          ...
          console.error('Error refreshing data after invite:', refreshError);
        }
  ...
  const handleBackToOnboarding = async () => {
    try {
      ...
      const apiUrl = process.env.NEXT_PUBLIC_COMPANY_API_URL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api';
      await axios.put(
        `${apiUrl}/onboarding/companies/${companyId}/onboarding/phases/3/steps/10`,
        { status: "completed" }
      );
                ...
                <div key={`weight-${key}`} className="bg-white rounded-lg p-2 shadow-md border border-gray-200 hover:shadow-lg transition-all duration-200 group w-full max-w-full">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div                       className={`px-2 py-1 rounded text-xs font-bold ${
                      Math.round(value * 100) >= 20 ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white' :
                      Math.round(value * 100) >= 10 ? 'bg-blue-100 text-blue-800' :
                  ...
                  <div className="relative mb-2">
                    ...
                    />
                    <div 
                      className="absolute top-1/2 transform -translate-y-1/2 w-4 h-4 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full shadow-md border-2 border-white pointer-events-none transition-all duration-200 group-hover:scale-110"
            ...
            {selectedGig && (
              <div className="flex justify-center">
                ...
                >
                            ...
                            <div className="flex items-center space-x-3 mb-3">
                              <div className={`p-2 rounded-lg ${
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  selectedGig?._id === gig._id ? "text-blue-900" : "text-gray-800"
                                }`}>
                                  {gig.title}
                                </h4>
                            ...
                            <div className="px-4 pb-4 border-t border-gray-200 bg-gray-50 overflow-hidden">
                              <div className="pt-4 space-y-4 text-sm overflow-hidden">
                                ...
                                {gig.availability && (
                                  <div>
                                    ...
                                    <p className="font-semibold">
                                      {gig.availability.schedule && gig.availability.schedule.length > 0 ? `${gig.availability.schedule.length} days/week` : 
                                       'Flexible'}
                                    </p>
                                  </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                  {(gig.targetRegion || gig.destination_zone) && (
                                    <div>
                                      <span className="text-gray-600 font-medium">Region:</span>
                                      <p className="font-semibold">{gig.targetRegion || gig.destination_zone}</p>
                                    </div>
                                  )}
                                  {gig.timezone && (
                                    <div>
                  ...
                  {/* Resize Handle */}
                  <div 
                    className={`flex-shrink-0 w-1 bg-gray-200 hover:bg-blue-400 cursor-col-resize transition-colors duration-200 rounded-full flex items-center justify-center group ${isResizing ? 'bg-blue-500' : ''}`}
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-4 text-xs text-gray-500 mt-1">
                                    ...
                                    {match.agentInfo?.languages && match.agentInfo.languages.length > 0 && (
                                      <span>🗣️ {match.agentInfo.languages.length} languages</span>
                                    )}
                                  </div>
                                  ...
                                  {match.agentInfo?.languages && match.agentInfo.languages.length > 0 && (
                                    <div className="mt-2">
                                      <div className="flex flex-wrap gap-1">
                                        {match.agentInfo.languages.slice(0, 4).map((lang: any, i: number) => (
                                          <span key={i} className="px-2 py-1 bg-purple-100 text-purple-800 rounded text-xs">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-full overflow-hidden">
                                    ...
                                    {match.timezoneMatch && (
                                      <div className="bg-white rounded-lg p-4 shadow-sm">
                                        ...
                                        <div className="text-xs text-gray-600">
                                          ...
                                          <p>Zone: {match.agentInfo?.timezone?.timezoneName || 'N/A'}</p>
                                    ...
                                    {match.regionMatch && (
                                      <div className="bg-white rounded-lg p-4 shadow-sm">
                                        ...
                                        <div className="text-xs text-gray-600">
                                          <p>Country: {match.agentInfo?.timezone?.countryName || 'N/A'}</p>
                                          <p>Code: {match.agentInfo?.timezone?.countryCode || 'N/A'}</p>
                                        </div>
                                      </div>
                                    )}
                                    ...
                                    {match.availabilityMatch && (
                                      <div className="bg-white rounded-lg p-4 shadow-sm">
                                        ...
                                        <div className="text-xs text-gray-600">
                                          <p>Schedule: {match.availabilityMatch?.details?.matchingDays?.length || 0} days/week</p>
                                        </div>
                                      </div>
                                    )}
                            ...
                            <div className="flex-1">
                              ...
                              <div className="mt-3 space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {agent.agentId?.professionalSummary?.keyExpertise?.slice(0, 5).map((skill: string, i: number) => (
