...
const CallScript = () => {
  ...
  // Vï¿½rifier l'ï¿½tat de l'ï¿½tape au chargement
  useEffect(() => {
    if (companyId) {
      checkStepStatus();
    }
  ...
  // Vï¿½rifier l'ï¿½tat de l'ï¿½tape quand les donnï¿½es changent
  useEffect(() => {
  ...
  const checkStepStatus = async () => {
    try {
      ...
      // Vï¿½rifier l'ï¿½tat de l'ï¿½tape 8 via l'API d'onboarding
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_COMPANY_API_URL}/companies/${companyId}/onboarding/step?phase=2&step=8`
      );
      ...
      // Vï¿½rifier aussi le localStorage pour la cohï¿½rence
      ...
      // Si l'ï¿½tape n'est pas marquï¿½e comme complï¿½tï¿½e mais que les informations de base sont prï¿½sentes,
      // marquer automatiquement l'ï¿½tape comme complï¿½tï¿½e localement
      if (hasBasicInfo() && !isStepCompleted) {
        console.log('?? Auto-completing step 8 locally because basic info is present');
        
        // Marquer l'ï¿½tape comme complï¿½tï¿½e localement
        setIsStepCompleted(true);
        
        // Mettre ï¿½ jour le localStorage avec l'ï¿½tape 8 marquï¿½e comme complï¿½tï¿½e
        ...
        // Notifier le composant parent CompanyOnboarding via un ï¿½vï¿½nement personnalisï¿½
      ...
      // En cas d'erreur API, vï¿½rifier le localStorage
      const storedProgress = localStorage.getItem('companyOnboardingProgress');
      if (storedProgress) {
        try {
          const progress = JSON.parse(storedProgress);
  ...
  const handleCompleteCallScript = async () => {
    try {
      ...
      // Marquer l'ï¿½tape 8 comme complï¿½tï¿½e
            ...
            <div className="p-6">
              <div className="space-y-4">
                {scripts
                  ...
                  ?.sections.map((section: { id: number; title: string; content: string; type: string; variations: number }) => (
                    <div
                      key={section.id}
