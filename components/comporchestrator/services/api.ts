...
export const phoneNumberService = {
  ...
  searchPhoneNumbers: async (
    ...
    try {
      ...
      let numbers: AvailablePhoneNumber[] = [];
      ...
      const numbersWithProvider = numbers.map(number => ({
        ...number,
        provider
      }));

      console.log(`✅ Found ${numbersWithProvider.length} numbers for ${countryCode}`);
      return numbersWithProvider;
    } catch (error) {
