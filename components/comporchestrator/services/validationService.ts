...
export const validationService = {
  ...
  async validateAddress(address: AddressFields): Promise<ValidationResponse> {
    try {
      const response = await axios.post<ValidationResponse>(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/validation/address`,
        address
      );
  ...
  async validateDocument(file: File): Promise<ValidationResponse> {
    try {
      ...
      const response = await axios.post<ValidationResponse>(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/validation/document`,
        formData,
        {
          headers: {
  ...
  async validateTextValue(value: string, criteria: {
    ...
    try {
      const response = await axios.post<ValidationResponse>(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/validation/text`,
        {
          value,
          criteria
        }
