...
const ApprovalPublishing = () => {
  ...
  useEffect(() => {
    // Fetch gigs first - they contain populated activities, industries, languages
    // These will be extracted from the populated gigs data
    fetchGigs();
    fetchCompanyDetails();
    
    // Keep these as fallback in case there are no gigs yet
    // They will be overridden by data extracted from populated gigs
    fetchActivities();
    fetchIndustries();
    fetchLanguages();
  }, []);
  ...
  const fetchCompanyDetails = async () => {
    try {
      ...
      // Use the company endpoint which returns populated data
      const response = await axios.get<CompanyResponse>(`${API_BASE_URL}/companies/${companyId}`);
      console.log('✅ Company fetched:', response.data.data || response.data);
  ...
  const fetchActivities = async () => {
    try {
      // Activities endpoint returns { success: true, data: [...] }
      const response = await axios.get<ActivitiesResponse>(`${API_BASE_URL}/activities`);
  ...
  const fetchIndustries = async () => {
    try {
      // Industries endpoint returns { success: true, data: [...] }
      const response = await axios.get<IndustriesResponse>(`${API_BASE_URL}/industries`);
      if (response.data && response.data.success && Array.isArray(response.data.data)) {
  ...
  const fetchLanguages = async () => {
    try {
      // Languages endpoint returns { success: true, data: [...] }
      const response = await axios.get<LanguagesResponse>(`${API_BASE_URL}/languages`);
      if (response.data && response.data.success && Array.isArray(response.data.data)) {
        console.log('✅ Languages fetched:', response.data.data);
  ...
  const fetchGigs = async () => {
    ...
    try {
      ...
      const apiUrl = `${API_BASE_URL}/gigs/company/${companyId}`;
      ...
      if (data.data) {
        console.log('✅ Found gigs data with populated relations, transforming...');
        
        // Extract unique activities, industries, and languages from populated gigs
        const allActivities: Activity[] = [];
        const allIndustries: Industry[] = [];
        const allLanguages: Language[] = [];
        
        data.data.forEach((gig: any) => {
          // Extract activities (already populated)
          if (gig.activities && Array.isArray(gig.activities)) {
            gig.activities.forEach((act: any) => {
              if (act && act._id && !allActivities.find(a => a._id === act._id)) {
                allActivities.push({
                  _id: act._id,
          ...
          // Extract industries (already populated)
          if (gig.industries && Array.isArray(gig.industries)) {
            gig.industries.forEach((ind: any) => {
              if (ind && ind._id && !allIndustries.find(i => i._id === ind._id)) {
                allIndustries.push({
                  _id: ind._id,
          ...
          // Extract languages from skills.languages (already populated)
          if (gig.skills?.languages && Array.isArray(gig.skills.languages)) {
            gig.skills.languages.forEach((langEntry: any) => {
              const lang = langEntry.language;
              if (lang && lang._id && !allLanguages.find(l => l._id === lang._id)) {
                allLanguages.push({
                  _id: lang._id,
                  name: lang.name || 'Unknown',
                  code: lang.code || lang.iso639_1 || '',
                  ...lang
                });
              }
            });
        ...
        // Update activities, industries, languages from populated gigs
        if (allActivities.length > 0) {
          console.log('✅ Extracted activities from populated gigs:', allActivities.length);
          setActivities(allActivities);
        }
        if (allIndustries.length > 0) {
          console.log('✅ Extracted industries from populated gigs:', allIndustries.length);
          setIndustries(allIndustries);
        }
        if (allLanguages.length > 0) {
          console.log('✅ Extracted languages from populated gigs:', allLanguages.length);
          setLanguages(allLanguages);
        }
        
        // Transformer les données pour correspondre à l'interface attendue
        // Les gigs contiennent déjà les données peuplées (activities, industries, languages, skills, etc.)
        const transformedGigs = data.data.map((gig: any) => {
          ...
          console.log('✅ Transformed gig with populated data:', transformed);
          return transformed;
        });
        
        console.log('✅ Setting gigs state with', transformedGigs.length, 'gigs (with populated relations)');
  ...
  const filteredGigs = gigs.filter(gig => {
    ...
    // Mapping des statuts pour la compatibilitï¿½
  ...
  const formatStatus = (status: string) => {
    ...
    // Si le statut n'est pas dans le mapping, formater en remplaï¿½ant les underscores
  ...
  const approveGig = async (gigId: string) => {
    try {
      ...
      // Use API_BASE_URL which points to the backend (localhost:5000/api)
      const apiUrl = `${API_BASE_URL}/gigs/${gigId}`;
  ...
  const markStep13AsCompleted = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/companies/${companyId}/onboarding/complete-last`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
  ...
  const checkAndUpdateStep13Status = async () => {
    try {
      ...
      if (!hasActiveGig) {
        console.log('🔍 No active gigs found - updating phase and step statuses');
        
        // Mark step 13 as in_progress
        // Use API_BASE_URL which points to the backend (localhost:5000/api)
        const stepResponse = await fetch(
          `${API_BASE_URL}/companies/${companyId}/onboarding/step?phase=4&step=13`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
        ...
        const phaseResponse = await fetch(
          `${API_BASE_URL}/companies/${companyId}/onboarding/current-phase`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
        ...
        if (stepResponse.ok && phaseResponse.ok) {
          console.log('✅ Step 13 and phase 4 statuses updated successfully');
        } else {
  ...
  const fetchSkillsData = async (gigData: any) => {
    try {
      ...
      if (gigData.skills?.professional) {
        gigData.skills.professional.forEach((skill: any) => {
          ...
          if (skillId) {
            ...
            console.warn('?? Could not extract professional skill ID from:', skill);
          }
        });
      }
      ...
      if (gigData.skills?.technical) {
        gigData.skills.technical.forEach((skill: any) => {
          ...
          if (skillId) {
            ...
            console.warn('?? Could not extract technical skill ID from:', skill);
          }
        });
      }
      ...
      if (gigData.skills?.soft) {
        gigData.skills.soft.forEach((skill: any) => {
          ...
          if (skillId) {
            ...
            console.warn('?? Could not extract soft skill ID from:', skill);
          }
        });
      }
      ...
      if (professionalIds.length > 0) {
        try {
          // Use API_BASE_URL which points to the backend (localhost:5000/api)
          const response = await fetch(`${API_BASE_URL}/skills/professional`);
          if (response.ok) {
              ...
              if (responseData.success && responseData.data && Array.isArray(responseData.data)) {
                ...
                professionalIds.forEach(skillId => {
                  ...
                  if (skill) {
                    ...
                    console.warn(`?? Professional skill ${skillId} not found`);
                  }
                });
              } else {
                console.warn('?? Invalid response format for professional skills');
              }
            } else {
              console.warn('?? Response is not JSON for professional skills');
            }
          } else {
            console.warn('?? Failed to fetch professional skills:', response.status, response.statusText);
          }
        } catch (error) {
          console.warn('?? Error fetching professional skills:', error);
        }
      }
      ...
      if (technicalIds.length > 0) {
        try {
          // Use API_BASE_URL which points to the backend (localhost:5000/api)
          const response = await fetch(`${API_BASE_URL}/skills/technical`);
          if (response.ok) {
            const contentType = response.headers.get('content-type');
            if (contentType && contentType.includes('application/json')) {
              const responseData = await response.json();
              ...
              if (responseData.success && responseData.data && Array.isArray(responseData.data)) {
                ...
                technicalIds.forEach(skillId => {
                  ...
                  if (skill) {
                    ...
                    console.warn(`?? Technical skill ${skillId} not found`);
                  }
                });
              } else {
                console.warn('?? Invalid response format for technical skills');
              }
            } else {
              console.warn('?? Response is not JSON for technical skills');
            }
          } else {
            console.warn('?? Failed to fetch technical skills:', response.status, response.statusText);
          }
        } catch (error) {
          console.warn('?? Error fetching technical skills:', error);
        }
      }
      ...
      if (softIds.length > 0) {
        try {
          // Use API_BASE_URL which points to the backend (localhost:5000/api)
          const response = await fetch(`${API_BASE_URL}/skills/soft`);
          if (response.ok) {
            const contentType = response.headers.get('content-type');
            if (contentType && contentType.includes('application/json')) {
              const responseData = await response.json();
              ...
              if (responseData.success && responseData.data && Array.isArray(responseData.data)) {
                ...
                softIds.forEach(skillId => {
                  ...
                  if (skill) {
                    ...
                    console.warn(`?? Soft skill ${skillId} not found`);
                  }
                });
              } else {
                console.warn('?? Invalid response format for soft skills');
              }
            } else {
              console.warn('?? Response is not JSON for soft skills');
            }
          } else {
            console.warn('?? Failed to fetch soft skills:', response.status, response.statusText);
          }
        } catch (error) {
          console.warn('?? Error fetching soft skills:', error);
        }
      }
  ...
  const fetchTimezoneData = async (gigData: any) => {
    try {
      ...
      if (!timezoneId) {
        console.log('?? No timezone ID found in gig data');
        return;
      }
          ...
          if (contentType && contentType.includes('application/json')) {
            ...
            if (responseData.success && responseData.data && Array.isArray(responseData.data)) {
              ...
              if (timezone) {
                ...
                console.warn(`?? Timezone ${timezoneId} not found`);
              }
            } else {
              console.warn('?? Invalid response format for timezones');
            }
          } else {
            console.warn('?? Response is not JSON for timezones');
          }
        } else {
          console.warn('?? Failed to fetch timezones:', response.status, response.statusText);
        }
      } catch (error) {
        console.warn('?? Error fetching timezones:', error);
      }
    } catch (error) {
  ...
  const fetchDestinationZoneData = async (gigData: any) => {
    try {
      ...
      if (!destinationZone) {
        console.log('?? No destination zone found in gig data');
        return;
      }
      ...
      try {
        ...
        // Use API_BASE_URL which points to the backend (localhost:5000/api)
        const response = await fetch(`${API_BASE_URL}/timezones/country/${countryCode}`);
        if (response.ok) {
          const contentType = response.headers.get('content-type');
          if (contentType && contentType.includes('application/json')) {
            const responseData = await response.json();
            ...
            if (responseData.success && responseData.data && Array.isArray(responseData.data) && responseData.data.length > 0) {
              ...
              console.warn('?? Invalid response format for destination zone timezones');
            }
          } else {
            console.warn('?? Response is not JSON for destination zone timezones');
          }
        } else {
          console.warn('?? Failed to fetch destination zone timezones:', response.status, response.statusText);
        }
      } catch (error) {
        console.warn('?? Error fetching destination zone timezones:', error);
      }
    } catch (error) {
  ...
  const previewGig = async (gigId: string) => {
    try {
      console.log('🔍 Previewing gig:', gigId);
      const companyId = Cookies.get('companyId');
      const userId = Cookies.get('userId');
      const gigIdCookie = Cookies.get('gigId');
  ...
  const addCommissionOption = () => {
    ...
    // Cette fonction peut ï¿½tre ï¿½tendue pour ajouter de nouvelles options de commission
  ...
  const addSchedule = () => {
    ...
    const newSchedule = {
      ...
      displayText: `${selectedDays.join(' et ')} de ${workingHours.start} jusqu'ï¿½ ${workingHours.end}`
    };
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await axios.put<{success: boolean; message?: string}>(
        `${API_BASE_URL}/gigs/${currentGigData._id}`,
        updateData,
        {
          headers: {
            'Content-Type': 'application/json'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 mt-1">{currentGigData.commission?.baseAmount ? `${currentGigData.commission.baseAmount} ${currentGigData.commission.currency?.symbol || 'ï¿½'}` : 'Not specified'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 mt-1">{currentGigData.commission?.bonusAmount ? `${currentGigData.commission.bonusAmount} ${currentGigData.commission.currency?.symbol || 'ï¿½'}` : 'Not specified'}</p>
                </div>
              </div>
              <div className="space-y-3">
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 mt-1">{currentGigData.commission?.transactionCommission?.amount ? `${currentGigData.commission.transactionCommission.amount} ${currentGigData.commission?.currency?.symbol || 'ï¿½'}` : 'Not specified'}</p>
                </div>
                <div>
