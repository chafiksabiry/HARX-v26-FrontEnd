...
export default class AudioLoopbackTest {
  ...
  async start() {
    ...
    try {
      ...
      // Note: Worklet path needs to be adjusted for Next.js
      try {
        const workletUrl = '/audio-processor.js'; // Adjust path as needed
        ...
        console.warn('⚠️ Worklet not available, using simplified mode:', workletError);
        // Fallback: direct audio passthrough without encoding
